	...
	var qodefSwiper = {
		...
		getOptions: function ( $holder, returnBreakpoints ) {
			var sliderOptions     = typeof $holder.data( 'options' ) !== 'undefined' ? $holder.data( 'options' ) : {},
				...
				paginationType    = sliderOptions.paginationType !== undefined && sliderOptions.paginationType !== '' ? sliderOptions.paginationType : 'fraction';
			...
			var slidesPerView1440 = sliderOptions.slidesPerView1440 !== undefined && sliderOptions.slidesPerView1440 !== '' ? parseFloat(
				sliderOptions.slidesPerView1440,
				10
				) : 5,
				slidesPerView1366 = sliderOptions.slidesPerView1366 !== undefined && sliderOptions.slidesPerView1366 !== '' ? parseFloat(
					sliderOptions.slidesPerView1366,
					10
				) : 4,
				slidesPerView1024 = sliderOptions.slidesPerView1024 !== undefined && sliderOptions.slidesPerView1024 !== '' ? parseFloat(
					sliderOptions.slidesPerView1024,
					10
				) : 3,
				slidesPerView768  = sliderOptions.slidesPerView768 !== undefined && sliderOptions.slidesPerView768 !== '' ? parseFloat(
					sliderOptions.slidesPerView768,
					10
				) : 2,
				slidesPerView680  = sliderOptions.slidesPerView680 !== undefined && sliderOptions.slidesPerView680 !== '' ? parseFloat(
					sliderOptions.slidesPerView680,
					10
				) : 1,
				slidesPerView480  = sliderOptions.slidesPerView480 !== undefined && sliderOptions.slidesPerView480 !== '' ? parseFloat(
			...
			return {
				on: {
					...
					init: function () {
						...
						qodef.body.trigger(
							'esmee_trigger_swiper_is_initialized',
							[$holder, options]
						);
						...
						if ( $holder.hasClass( 'qodef-pagination-fractions-with-line' ) ) {
							var totalSlide  = $holder.find('.swiper-slide:not(.swiper-slide-duplicate)').length;
							$holder.find( '.swiper-total' ).html( totalSlide);
						}
					},
					slideChange: function () {
						if ( $holder.hasClass( 'qodef-pagination-fractions-with-line' ) ) {
							...
							$holder.find('.swiper-active').html( activeSlide + 1 );
							$holder.find('.swiper-total').html( totalSlide);
						}
					}
				}
...
(function ( $ ) {
	...
	$( window ).on(
		...
		function () {
			...
			if ( $( '.qodef-woo-single-image' ).length ) {
				mutationObserver.observe(
					$( '.qodef-woo-single-image' )[0],
					{
						attributes: true,
						characterData: true,
						childList: true,
