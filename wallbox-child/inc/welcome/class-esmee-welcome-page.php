...
if ( ! class_exists( 'Esmee_Welcome_Page' ) ) {
	/**
	 * Welcome page theme class
	 */
	class Esmee_Welcome_Page {
		private static $instance;

		/**
		 * @return Esmee_Welcome_Page
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_esmee_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_esmee_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_esmee_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'esmee_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			...
			remove_submenu_page( 'themes.php', 'esmee_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( ESMEE_ROOT_DIR . '/screenshot.png' ) ? ESMEE_ROOT . '/screenshot.png' : ESMEE_ROOT . '/screenshot.jpg';

			esmee_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			...
			if ( 'appearance_page_esmee_welcome_page' === $hook ) {
				wp_enqueue_style( 'esmee-welcome-page-style', ESMEE_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}

	Esmee_Welcome_Page::get_instance();
}
